...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2 class="fw-bold text-white">Editar Pedido #{{ $pedido->id }}</h2>
    ...
    <div class="card-body">
        <form action="{{ route('admin.pedidos.update', $pedido) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select name="venta_id" id="venta_id" class="form-select" required>
                    ...
                    @foreach($ventas as $venta)
                        <option value="{{ $venta->id }}" {{ $pedido->venta_id == $venta->id ? 'selected' : '' }}>
            ...
            <div class="mb-3">
                ...
                <textarea name="nota" id="nota" class="form-control">{{ old('nota', $pedido->nota) }}</textarea>
                @error('nota')
                    <small class="text-danger">{{ $message }}</small>
                @enderror
            </div>
